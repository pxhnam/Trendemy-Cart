function toast({ message = "", type = "info", duration = 3 }) {
    ...
    toast.classList.add("notify", `notify--${type}`);
    toast.style.animation = `slideDown ease .3s, slideUp linear .3s ${delay}s forwards`;
    toast.innerHTML = `<div class="notify-icon">${icons[type]}</div>
                           <div class="notify-body"><p class="notify-msg">${message}</p></div>`;
